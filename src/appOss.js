...
export default {
  ...
  ossUploadFile (option) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let fileName = dateTime + '_' + randomStr + extensionName // 文件名字（相对于根目录的路径 + 文件名）
      // 执行上传
      self.createOssClient().then(client => {
        // 异步上传,返回数据
        resolve({
          fileName: file.name,
          fileUrl: 'https://scmall-dev.oss-cn-hangzhou.aliyuncs.com/' + fileName
        })
        // 上传处理
        // 分片上传文件
